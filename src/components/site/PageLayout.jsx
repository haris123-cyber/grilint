...
export function PageLayout({ children }) {
  ...
  return (
    <main ref={mainRef} className="min-h-screen bg-background text-foreground overflow-x-clip relative z-0">
      <div ref={contentAboveRef} className="bg-background relative z-20 origin-bottom transform-gpu pb-10 shadow-2xl">
        <Nav />
        {children}
      </div>

      {/* Footer Reveal Wrapper */}
      <div ref={footerTriggerRef} className="relative z-0 h-screen w-full" style={{ clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)" }}>
