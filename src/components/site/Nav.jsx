...
export function Nav() {
  ...
  return (
    <>
      {/* ── Top bar ── */}
      <header
        className={`fixed top-0 inset-x-0 z-[60] flex items-center justify-between transition-all duration-500 ${
          scrolled ? "px-5 md:px-8 py-3" : "px-6 md:px-10 py-5"
        } ${
          scrolled || open
      ...
      >
        ...
        <div className="flex items-center gap-4 z-[61]">
          ...
          >
            <span
              className={`block h-px w-5 bg-foreground transition-all duration-300 ease-in-out ${
                open ? "rotate-45 translate-y-[0.5px]" : "-translate-y-[3px]"
              }`}
            />
            <span
              className={`block h-px bg-foreground transition-all duration-300 ease-in-out ${
                open ? "w-5 -rotate-45 -translate-y-[0.5px]" : "w-3 translate-y-[3px]"
              }`}
            />
          </button>
        </div>
      ...
      {/* ── Slide-in menu panel ── */}
      <div
        className={`fixed top-0 right-0 h-full w-full z-[58] flex flex-col px-8 md:px-16 pt-28 pb-12 transition-all duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] ${
      ...
      >
        {/* Left accent line */}
        <div
          className={`absolute left-0 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-mint/30 to-transparent transition-opacity duration-700 ${
        ...
        <nav className="flex flex-col gap-1 overflow-y-auto flex-1 min-h-0 pr-4 no-scrollbar">
          {links.map((l, i) => (
            <NavLink
              ...
              onClick={close}
              className={({ isActive }) =>
                `group flex items-end gap-4 py-3 border-b border-border/30 hover:text-mint transition-all duration-500 ease-out ${
                  isActive ? "text-mint" : "text-foreground"
        ...
        {/* Bottom row */}
        <div
          className={`flex flex-col md:flex-row items-start md:items-end justify-end gap-6 transition-all duration-500 mt-6 shrink-0 ${
            open ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          }`}
          style={{ transitionDelay: open ? "520ms" : "0ms" }}
        >
          <div className="space-y-1">
            <p className="text-[10px] uppercase tracking-[0.3em] text-mint font-mono">
              [ Get in touch ]
            </p>
            <a
